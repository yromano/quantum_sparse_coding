...
for M_id, M_val in enumerate(M_vec):
    for noise_id, b_noise_coef in enumerate(noise_vec):
        for exp_id in range(num_exps):
            print("M_id: " + str(M_id+1) + " noise_id: " + str(noise_id+1) + "/" + str(len(noise_vec)) + " exp_id: " + str(exp_id+1) + "/" + str(num_exps))
            sys.stdout.flush()
            
            # %% Form the system of equations
            [A, b, x0, A_test, b_test] = \
                RLS.generate_random_linear_system(N=N, M=M_val, K=K,
...
df = results
df = df[df[["Experiment", "Method","M","Infeasible X-Recovery L2 Err"]]["Method"]=="Lasso"]
sns.lineplot(x = "M", y = "Infeasible X-Recovery L2 Err", data = df[["Experiment", "Method","M","Infeasible X-Recovery L2 Err"]])
plt.show()

df = results
df = df[df[["Experiment", "Method","M","Infeasible X-Recovery L0 Err"]]["Method"]=="Lasso"]
sns.lineplot(x = "M", y = "Infeasible X-Recovery L0 Err", data = df[["Experiment", "Method","M","Infeasible X-Recovery L0 Err"]])
plt.show()
